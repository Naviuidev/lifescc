...
final class WeightLossRepository
{
    public static function create(array $row): int
    {
        $pdo = Database::connection();
        $sql = 'INSERT INTO weight_loss (full_name, phone, location_id, message, source_page)
                VALUES (:full_name, :phone, :location_id, :message, :source_page)';
    ...
    public static function allLatestFirst(): array
    {
        $pdo = Database::connection();
        $stmt = $pdo->query(
            'SELECT id, full_name, phone, location_id, message, created_at,
    ...
    public static function find(int $id): ?array
    {
        ...
        $stmt = $pdo->prepare(
            'SELECT id, full_name, phone, location_id, message, created_at,
                    status, follow_up, visited, contacted, customer_note, source_page
             FROM weight_loss WHERE id = :id LIMIT 1'
        );
        $stmt->execute([':id' => $id]);
    ...
    public static function update(int $id, array $fields): bool
    {
        ...
        $sql = 'UPDATE weight_loss SET ' . implode(', ', $parts) . ' WHERE id = :id';
        $stmt = $pdo->prepare($sql);
