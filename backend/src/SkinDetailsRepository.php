...
final class SkinDetailsRepository
{
    public static function create(array $row): int
    {
        $pdo = Database::connection();
        $sql = 'INSERT INTO skin_details (full_name, phone, location_id, message, service_label, source_page)
    ...
    public static function find(int $id): ?array
    {
        ...
        $stmt = $pdo->prepare(
            'SELECT id, full_name, phone, location_id, message, service_label, created_at,
                    status, follow_up, visited, contacted, customer_note, source_page
             FROM skin_details WHERE id = :id LIMIT 1'
        );
        $stmt->execute([':id' => $id]);
    ...
    public static function update(int $id, array $fields): bool
    {
        ...
        $sql = 'UPDATE skin_details SET ' . implode(', ', $parts) . ' WHERE id = :id';
        $stmt = $pdo->prepare($sql);
