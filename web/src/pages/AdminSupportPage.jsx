...
function branchLabel(branchId) {
  const b = LIFESCC_BRANCHES.find((x) => x.id === branchId)
...
function matchesSupportDateRange(row, from, to) {
  if (!from && !to) return true
  const key = supportDateKey(row.created_at)
  if (!key) return true
...
export default function AdminSupportPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        setError(typeof data.error === 'string' ? data.error : 'Failed to load support requests')
        setSupports([])
        return
      }
  ...
  const filtered = useMemo(() => {
    return supports.filter(
      (row) =>
        ...
        matchesSupportDateRange(row, dateFrom, dateTo) &&
        matchesStatusFilter(row, statusFilter),
    )
  }, [supports, searchQuery, dateFrom, dateTo, statusFilter])
  ...
  const mergeRow = (updated) => {
    setSupports((prev) => prev.map((s) => (s.id === updated.id ? { ...s, ...updated } : s)))
  }

  const handlePatch = async (id, body) => {
    setPatchError(null)
    try {
      const row = await patchSupport(id, body)
      mergeRow(row)
    } catch (e) {
    ...
    <div className="admin-slots">
      <header className="admin-slots__header">
        <div className="admin-slots__header-intro">
          <h1 className="admin-slots__title">Customer support</h1>
          <p className="admin-slots__lead">Requests from the Contact us — Customer Support flow</p>
        </div>
        <div className="admin-slots__toolbar">
          <div className="admin-slots__search-wrap">
            <label className="admin-slots__search-label">
              ...
              <input
                ...
                placeholder="Search name, email, mobile, branch, query…"
                value={searchQuery}
          ...
          <div className="admin-slots__actions">
            <div className="admin-slots__status-wrap">
              <label className="admin-slots__status-label visually-hidden" htmlFor="admin-support-status">
                Status
              </label>
              <select
      ...
      <div className="admin-table-card">
        <div className="admin-table-scroll admin-table-scroll--supports">
          <table className="admin-data-table">
            ...
            <tbody>
              ...
              {!loading && supports.length === 0 && !error ? (
                <tr>
                  <td colSpan={COL_COUNT} className="admin-data-table__empty">
              ...
              {!loading && supports.length > 0 && filtered.length === 0 ? (
                <tr>
                  <td colSpan={COL_COUNT} className="admin-data-table__empty">
                ...
                return (
                  <tr key={row.id}>
                    <td className="admin-data-table__td-user">
                      <AdminUserContactDropdown fullName={name} email={row.email} mobile={row.mobile ?? '—'} />
                    </td>
                    <td className="admin-data-table__nowrap">{branchLabel(row.branch_id)}</td>
                    <td>
                      <span className="admin-contacts__msg" title={q}>
                        {qShort || '—'}
                      </span>
                    </td>
                    <td>
        ...
        <footer className="admin-table-footer">
          ...
          <div className="admin-table-footer__range">
            <span>
              ...
              {filtered.length !== supports.length ? (
                <span className="admin-table-footer__total"> ({supports.length} total)</span>
              ) : null}
            </span>
            <button
              type="button"
